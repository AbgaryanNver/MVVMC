...
final class AnchorMaker {
    ...
    private final class AnchorDescription {
        ...
        var identifier: String {
            ("\(anchor.rawValue)_to_\(relatedAnchor.rawValue)")
        }

        private(set) var anchor: Anchors
        private(set) var applied: Bool = false

        init(_ anchor: Anchors, connectionType: ConstraintConnectionType = .equal, offset: CGFloat = 0, priority: UILayoutPriority? = nil) {
            self.anchor = anchor
     ...
     */
    @discardableResult
    func equalTo(_ view: UIView, anchor: Anchors? = nil, _ offset: CGFloat = 0, priority: UILayoutPriority? = nil) -> AnchorMaker {
        for description in descriptions where !description.applied {
    ...
    private static func activateDimensionAnchor(view: UIView, _ description: AnchorDescription) {
        if let selfAnchorPath = description.anchor.dimensionKeyPath() {
            ...
            switch description.connectionType {
                ...
                case .greater:
                    constraint = view[keyPath: selfAnchorPath].constraint(greaterThanOrEqualToConstant: description.offset)
                case .less:
